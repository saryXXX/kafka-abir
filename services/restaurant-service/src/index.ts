...
// gRPC Handlers
const getRestaurantsHandler = (call: any, callback: any) => {
  try {
...
const getMenuHandler = (call: any, callback: any) => {
...
const checkAvailabilityHandler = (call: any, callback: any) => {
    ...
    await consumer.run({
      eachMessage: async ({ message }) => {
        ...
        if (event.type === 'ORDER_CREATED') {
          const { orderId, restaurant_id: restaurantId, items, totalPrice } = event.data;
